...
Use the `/v1/collection/categorize/` endpoint when:
...
Send a `POST` request to the `/v1/collection/categorize/` endpoint with the required parameters in the body.
...
axios.post('https://api.wetrocloud.com/v1/collection/categorize/', {
  resource: "match review: John Cena vs. The Rock.",
  type: "text",
  json_schema: "{'label':'string'}",
  json_schema_rules: [
