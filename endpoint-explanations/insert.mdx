...
Use the `/v1/insert/resource/` endpoint when you need to:
...
To insert a resource into a collection, make a `POST` request to the `/v1/insert/resource/` endpoint with the required parameters in the body.
...
<CodeGroup>
    ```bash cURL
    curl --request POST \
      --url https://api.wetrocloud.com/v1/insert/resource/ \
    ...
    const url = 'https://api.wetrocloud.com/v1/insert/resource/';
    const headers = {
        'Authorization': 'Token <api-key>',
