...
<Info>
  At a glance: <br /> Use the `/resource/insert/` endpoint to add resources
  (URLs, files, text or JSON) to a collection for indexing and retrieval. Simply
  provide a `collection_id`, `resource`, and `type`.
</Info>
...
<Note>
  {" "}
  To use the `file` type, all files must be provided as URLs. You can consider services
  like Amazon S3, Cloudinary, or similar platforms to host your files and provide
  their URLs to the API.{" "}
</Note>
